...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        self.a_levels.append(set(new_a_nodes))
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level - 1]:
            new_s_nodes.update(a_node.effnodes)

            for s_node in new_s_nodes:
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if set(node_a1.action.effect_add).issubset(set(node_a2.action.precond_neg)) \
        or set(node_a2.action.effect_add).issubset(set(node_a1.action.precond_neg)):
            return True
        else:
            return False
        ...
        for goal in self.problem.goal:
            for level in range(len(self.s_levels)):
                ...
                for node_s in self.s_levels[level]:
                    if (node_s.symbol == goal and node_s.is_pos):
                        level_sum += level
                        goal_found_flag = True
                        break
                if goal_found_flag is True:
                    break
        return level_sum
